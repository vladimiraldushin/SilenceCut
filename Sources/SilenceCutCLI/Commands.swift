...
enum Commands {
    ...
    static func validate(_ args: Arguments) throws {
        ...
        // Перебивки лежат на ОДНОЙ дорожке, поэтому наехавшую сборщик композиции
        // пропускает. Молча теряющаяся картинка — худший из возможных исходов, ловим
        // заранее. Графика так не проверяется: её слой раскладывается на несколько
        // дорожек, и наложение там — рабочий случай (строка под подписями).
